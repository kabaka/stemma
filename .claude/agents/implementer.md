...
- Single responsibility: turn an approved unit-of-work contract and plan into
  ...
  (`software-architect`), choose the sequence (`planner`), author tests (`test-engineer`),
...
- Load `conventional-commits` when writing a commit message, and the
  ...
  input, or anything that runs on another machine — escalating to `security-privacy-reviewer` on its
