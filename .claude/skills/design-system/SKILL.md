...
> **Stemma note.** Stemma already has a committed dark clinical design system and a
> colourblind-safe palette (see `src/ui/` and the `frontend-engineer` /
> `accessibility-reviewer` agents). Use this skill to work *within* and extend that
> system consistently — not to invent a new one. Stemma does **not** install the
> kit's separate `visual-qa/` harness or an `.ai-dlc/stack-binding.json`; verify UI
> in the running app (`npm run dev`, Chromium/Playwright are available) and gate with
> `accessibility-reviewer`. Ignore references below to `off-token-lint` / the
> visual-QA scripts as installed tooling.
